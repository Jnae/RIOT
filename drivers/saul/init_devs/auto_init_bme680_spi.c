 ...
 */
#define BME680_COMMON_INFO_NUMOF   ARRAY_SIZE(bme680_spi_saul_info)
...
void auto_init_bme680_spi(void)
{
    ...
    for (unsigned i = 0; i < BME680_COMMON_NUMOF; i++) {
        ...
        bme680_spi_t *dev = &bme680_common_devs_saul[i];
        if (bme680_spi_init(dev, &bme680_spi_params[i]) < 0) {
